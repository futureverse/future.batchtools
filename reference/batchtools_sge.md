...
- template:
  ...
  If not found, it defaults to the `templates/sge.tmpl` part of this
  package (see below).
...
which are used to interact with the SGE job scheduler. This requires
that SGE commands `qsub`, `qstat`, and `qdel` are available on the
current machine.
