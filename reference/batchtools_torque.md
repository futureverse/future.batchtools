...
- template:
  ...
  If not found, it defaults to the `templates/torque.tmpl` part of this
  package (see below).
...
which are used to interact with the TORQUE/PBS job scheduler. This
requires that TORQUE/PBS commands `qsub`, `qselect`, and `qdel` are
...
This template and the built-in
[`batchtools::makeClusterFunctionsTORQUE()`](https://batchtools.mlr-org.com/reference/makeClusterFunctionsTORQUE.html)
have been verified to work on the following PBS/TORQUE HPC cluster;

1.  PBSPro 2024.1.2, Rocky 8 Linux, Lustre global filesystem (September
    2025)
